    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2 flex-wrap mb-1">
          <span className="font-medium text-sm text-[#2E2E2E]">{comment.name}</span>
          <span className="text-[10px] text-[#9B8B7A]">{comment.date}</span>
        </div>
        {comment.rating > 0 && (
        ...
        <p className="text-sm text-[#6B6B6B] leading-relaxed">{comment.message}</p>
      </div>
    </motion.div>
  );
}
  ...
  const [comments, setComments] = useState<Comment[]>([
    {
      id: "seed-1",
      name: "Sana R.",
      message: "Absolutely gorgeous piece! The quality of yarn is premium and the stitching is incredibly neat. Came beautifully packaged too.",
      rating: 5,
      date: "Feb 28, 2026",
      initials: "SR",
    },
    {
      id: "seed-2",
      name: "Mehak A.",
      message: "Ordered a custom bouquet as a birthday gift — the recipient was over the moon! Will definitely order again.",
      rating: 5,
      date: "Feb 14, 2026",
      initials: "MA",
    },
  ...
  return (
    <section className="mt-16">
      <div className="flex items-center gap-3 mb-6">
        <MessageSquare size={20} className="text-[#C4A484]" />
        <h2 className="font-playfair text-2xl text-[#2E2E2E] font-semibold">
      ...
      <form
        ...
        className="bg-white rounded-2xl p-5 mb-6 shadow-boutique space-y-4 border border-[#F0E6D3]"
      >
        <p className="text-sm font-medium text-[#2E2E2E]">Leave a review</p>
