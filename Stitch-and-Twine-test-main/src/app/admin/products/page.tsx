  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      ...
      <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-playfair text-2xl sm:text-3xl text-[var(--text-primary)]">Products</h1>
      ...
      <div className="bg-[var(--surface)] rounded-2xl shadow-boutique overflow-hidden">
        {/* Desktop header */}
        <div className="hidden lg:grid grid-cols-12 gap-4 px-5 py-3 border-b border-[var(--border-color)] text-xs uppercase tracking-widest text-[var(--text-secondary)] font-medium">
          ...
          <AnimatePresence>
            {filtered.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="text-[var(--text-secondary)] text-sm">No products found</p>
              </div>
            ) : (
              filtered.map((product) => (
                ...
                >
                  {/* Desktop row */}
                  <div className="hidden lg:grid grid-cols-12 gap-4 px-5 py-3.5 items-center hover:bg-[var(--background)] transition-colors">
                    <div className="col-span-5 flex gap-3 items-center">
                      <div className="relative w-10 h-10 rounded-lg overflow-hidden flex-shrink-0 bg-[var(--background)]">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-[var(--text-secondary)] truncate">{product.slug}</p>
                      </div>
                    </div>
                    <div className="col-span-2">
                      <span className="px-2 py-0.5 rounded-full text-xs bg-[var(--background)] text-[var(--text-secondary)]">{product.category}</span>
                    </div>
                    <div className="col-span-2">
                      <p className="text-sm font-medium text-[var(--text-primary)]">{formatPrice(product.price)}</p>
                      {product.comparePrice && <p className="text-xs text-[var(--text-secondary)] line-through">{formatPrice(product.comparePrice)}</p>}
                    </div>
                  ...
                  {/* Mobile card */}
                  <div className="lg:hidden p-4 flex gap-3 items-start">
                    <div className="relative w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 bg-[var(--background)]">
                      <Image src={getSafeImageSrc(product.images[0])} alt={product.name} fill className="object-cover" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-[var(--text-primary)] truncate">{product.name}</p>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="px-2 py-0.5 rounded-full text-[10px] bg-[var(--background)] text-[var(--text-secondary)]">{product.category}</span>
                        <span className="text-sm font-semibold text-[var(--text-primary)]">{formatPrice(product.price)}</span>
                      </div>
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center gap-3 text-xs text-[var(--text-secondary)]">
                          <span>Stock: <span className={product.stock === 0 ? "text-red-400" : product.stock <= 5 ? "text-orange-400" : "text-green-500"}>{product.stock}</span></span>
                          {product.rating && (
                        ...
                        <div className="flex gap-1">
                          <Link href={`/admin/products/edit/${product.id}`}
                            className="w-7 h-7 rounded-lg bg-[var(--background)] flex items-center justify-center">
                            <Edit size={12} className="text-[var(--text-secondary)]" />
                          </Link>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-playfair text-lg text-[var(--text-primary)]">Delete Product?</h3>
                <button onClick={() => setConfirmDelete(null)} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
                  <X size={18} />
                </button>
              </div>
              <p className="text-sm text-[var(--text-secondary)] mb-5">This product will be removed from the store. This action cannot be undone.</p>
